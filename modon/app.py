...
def fmt_m(value: float | None) -> str:
    """Format a value in AED as millions with one decimal place."""
    if value is None:
        return "—"
    return f"{value / 1_000_000:,.1f} M"


st.set_page_config(page_title="Modon – Market Analysis", layout="wide")
...
# Apply developer filter to data used by all tabs
...
with tab_overview:
    ...
    k2.metric(
        "Active Projects",
        f"{(projects['PROJECT_STATUS'] == 'ACTIVE').sum():,}",
    )
    k3.metric(
        "Units in Pipeline",
        f"{int(projects['CNT_UNIT'].cast(pl.Float64, strict=False).sum()):,}",
    )
    k4.metric(
        "Total Sales Value (M AED)",
        fmt_m(transactions['TRANS_VALUE'].sum()),
    )
    k5.metric(
        "Median Price/sqm (AED)",
        f"{transactions['PRICE_PER_SQM'].median():,.0f}",
    )
    ...
    with col1:
        st.markdown("**Monthly Sales Volume (M AED) – All Market**")
        st.line_chart(
            transactions.group_by("MONTH")
            .agg((pl.col("TRANS_VALUE").sum() / 1_000_000).alias("Sales Value (M AED)"))
            .sort("MONTH").to_pandas().set_index("MONTH")
        )

    with col2:
        st.markdown("**Monthly Transaction Count – All Market**")
        st.line_chart(
            transactions.group_by("MONTH")
    ...
    st.markdown("**Median Price/sqm by Area**")
    if "AREA_EN" in transactions.columns:
        ...
        st.bar_chart(by_area.to_pandas().set_index("AREA_EN")["Median Price/sqm"])

    col3, col4 = st.columns(2)
    with col3:
        st.markdown("**Active vs Pending Projects**")
        status_counts = (
            projects.group_by("PROJECT_STATUS")
            .agg(pl.len().alias("Count"))
            .to_pandas().set_index("PROJECT_STATUS")
        )
        st.bar_chart(status_counts)

    with col4:
        st.markdown("**Top 10 Areas by Transaction Volume**")
        if "AREA_EN" in transactions.columns:
            top_areas = (
                transactions.filter(pl.col("AREA_EN").is_not_null())
                .group_by("AREA_EN")
...
with tab_dev:
    if selected_developer == "— All —":
        ...
        col1, col2 = st.columns(2)
        with col1:
            st.subheader("Top 15 by Total Projects")
            top15 = leaderboard.head(15).select(["DEVELOPER_EN", "total_projects"]).to_pandas()
            st.bar_chart(top15.set_index("DEVELOPER_EN"))
        with col2:
            st.subheader("Top 15 by Sales Value")
            top15_sales = (
            ...
            st.bar_chart(top15_sales.set_index("DEVELOPER_EN"))
        ...
        st.dataframe(proj_display.to_pandas(), width="stretch", hide_index=True)
        ...
        if project_names:
            ...
            if selected_project != "— Select —":
                ...
                with col_a:
                    st.markdown("**Monthly Sales Volume (AED)**")
                    if proj_tx.height > 0:
                        st.line_chart(
                ...
                with col_b:
                    st.markdown("**Median Price per sqm over time**")
                    if proj_tx.height > 0:
                        st.line_chart(
                            proj_tx.group_by("MONTH")
                ...
                if proj_rn.height > 0:
                    st.markdown("**Annual Rent Contracts**")
                    st.line_chart(
...
with tab_tx:
    ...
    k4.metric(
        "Avg Transaction Value (M AED)",
        fmt_m(t['TRANS_VALUE'].mean()) if t.height > 0 else "—",
    )
    ...
    with col1:
        st.markdown("**Monthly Sales Volume (M AED)**")
        if t.height > 0:
            st.line_chart(
                t.group_by("MONTH")
                .agg((pl.col("TRANS_VALUE").sum() / 1_000_000).alias("Sales Value (M AED)"))
    ...
    st.markdown("**Median Price/sqm by Month**")
    if t.height > 0:
        st.line_chart(
            t.group_by("MONTH")
            .agg(pl.col("PRICE_PER_SQM").median().alias("Median AED/sqm"))
...
with tab_prop:
    ...
    if prop_tx.height > 0 and "PROP_TYPE_EN" in prop_tx.columns:
        ...
        st.dataframe(by_type.to_pandas(), width="stretch", hide_index=True)

        col1, col2 = st.columns(2)
        with col1:
            st.markdown("**Transaction Count by Property Type**")
            st.bar_chart(by_type.select(["PROP_TYPE_EN", "Transactions"]).to_pandas().set_index("PROP_TYPE_EN"))
        with col2:
            st.markdown("**Median Price/sqm by Property Type**")
            st.bar_chart(by_type.select(["PROP_TYPE_EN", "Median Price/sqm"]).to_pandas().set_index("PROP_TYPE_EN"))

        st.markdown("**Price/sqm Trend by Property Type**")
        prop_types = by_type["PROP_TYPE_EN"].drop_nulls().to_list()
        ...
        if selected_types:
            trend = (
                ...
                .sort("MONTH")
                .to_pandas()
                .pivot(index="MONTH", columns="PROP_TYPE_EN", values="Median AED/sqm")
            )
            st.line_chart(trend)
    else:
...
with tab_rent:
    ...
    k4.metric(
        "Avg Annual Contract (AED)",
        f"{r['ANNUAL_AMOUNT'].mean():,.0f}" if r.height > 0 else "—",
    )
    ...
    with col1:
        st.markdown("**Monthly Rent Volume (M AED)**")
        if r.height > 0:
            st.line_chart(
                r.group_by("MONTH")
                .agg((pl.col("ANNUAL_AMOUNT").sum() / 1_000_000).alias("Annual Rent (M AED)"))
    ...
    with col2:
        st.markdown("**Monthly Contract Count**")
        if r.height > 0:
            st.line_chart(
                r.group_by("MONTH")
    ...
    if r.height > 0 and "PROP_TYPE_EN" in r.columns:
        ...
        st.dataframe(by_type_rent.to_pandas(), width="stretch", hide_index=True)

        st.markdown("**Median Rent/sqm Trend by Property Type**")
        rent_types = by_type_rent["PROP_TYPE_EN"].drop_nulls().to_list()
        ...
        if selected_rent_types:
            rent_trend = (
                ...
                .sort("MONTH")
                .to_pandas()
                .pivot(index="MONTH", columns="PROP_TYPE_EN", values="Median Rent/sqm")
            )
            st.line_chart(rent_trend)
...
with tab_supply:
    ...
    with col1:
        ...
        if "LAND_TYPE_EN" in lands.columns:
            ...
            st.dataframe(land_type_counts.to_pandas(), width="stretch", hide_index=True)
    ...
    with col2:
        ...
        if "PROP_SUB_TYPE_EN" in lands.columns:
            ...
            st.bar_chart(sub_type_counts.to_pandas().set_index("PROP_SUB_TYPE_EN"))

    st.divider()
    st.markdown("**Units Pipeline by Completion Year**")
    if "END_DATE" in projects.columns:
        ...
        st.bar_chart(pipeline_by_year.to_pandas().set_index("Completion Year")["Units"])

    st.markdown("**Completion Progress Distribution**")
    if "PERCENT_COMPLETED" in projects.columns:
        ...
        if completed.height > 0:
            ...
            st.bar_chart(bins_df.to_pandas().set_index("Completion Band"))
